...
#include <sstream>
#include <memory>
#include "../../../applications/agentquartet/common/AQFilterRules_m.h"
...
void Ace::copy(const Ace& other)
{
    ...
    this->filterRule = other.filterRule;
...
void Ace::parsimPack(omnetpp::cCommBuffer *b) const
{
    ...
    doParsimPacking(b,this->filterRule);
...
void Ace::parsimUnpack(omnetpp::cCommBuffer *b)
{
    ...
    doParsimUnpacking(b,this->filterRule);
...
const char * Ace::getFilterRule() const
{
    return this->filterRule.c_str();
}

void Ace::setFilterRule(const char * filterRule)
{
...
unsigned int AceDescriptor::getFieldTypeFlags(int field) const
{
    ...
    static unsigned int fieldTypeFlags[] = {
        ...
        FD_ISEDITABLE,    // FIELD_filterRule
...
int AceDescriptor::findField(const char *fieldName) const
{
    ...
    if (fieldName[0] == 'f' && strcmp(fieldName, "filterRule") == 0) return base+1;
    if (fieldName[0] == 's' && strcmp(fieldName, "statistics") == 0) return base+2;
    return basedesc ? basedesc->findField(fieldName) : -1;
}
...
std::string AceDescriptor::getFieldValueAsString(void *object, int field, int i) const
{
    ...
    switch (field) {
        ...
        case FIELD_filterRule: return oppstring2string(pp->getFilterRule());
...
bool AceDescriptor::setFieldValueAsString(void *object, int field, int i, const char *value) const
{
    ...
    switch (field) {
        ...
        case FIELD_filterRule: pp->setFilterRule((value)); return true;
        default: return false;
    }
}
...
Acls::Acls() : ::inet::FieldsChunk()
{
}

Acls::Acls(const Acls& other) : ::inet::FieldsChunk(other)
{
    copy(other);
}
...
{
    delete [] this->acl;
    this->acl = (other.acl_arraysize==0) ? nullptr : new Acl[other.acl_arraysize];
    acl_arraysize = other.acl_arraysize;
    for (size_t i = 0; i < acl_arraysize; i++) {
        this->acl[i] = other.acl[i];
    }
}
...
void Acls::parsimPack(omnetpp::cCommBuffer *b) const
{
    ...
    b->pack(acl_arraysize);
    doParsimArrayPacking(b,this->acl,acl_arraysize);
}
...
void Acls::parsimUnpack(omnetpp::cCommBuffer *b)
{
    ...
    delete [] this->acl;
    b->unpack(acl_arraysize);
    if (acl_arraysize == 0) {
        this->acl = nullptr;
    } else {
        this->acl = new Acl[acl_arraysize];
        doParsimArrayUnpacking(b,this->acl,acl_arraysize);
    }
}

size_t Acls::getAclArraySize() const
{
    return acl_arraysize;
}

const Acl& Acls::getAcl(size_t k) const
{
    if (k >= acl_arraysize) throw omnetpp::cRuntimeError("Array of size acl_arraysize indexed by %lu", (unsigned long)k);
    return this->acl[k];
}

void Acls::setAclArraySize(size_t newSize)
{
    handleChange();
    Acl *acl2 = (newSize==0) ? nullptr : new Acl[newSize];
    size_t minSize = acl_arraysize < newSize ? acl_arraysize : newSize;
    for (size_t i = 0; i < minSize; i++)
        acl2[i] = this->acl[i];
    delete [] this->acl;
    this->acl = acl2;
    acl_arraysize = newSize;
}

void Acls::setAcl(size_t k, const Acl& acl)
{
    if (k >= acl_arraysize) throw omnetpp::cRuntimeError("Array of size  indexed by %lu", (unsigned long)k);
    handleChange();
    this->acl[k] = acl;
}

void Acls::insertAcl(size_t k, const Acl& acl)
{
    handleChange();
    if (k > acl_arraysize) throw omnetpp::cRuntimeError("Array of size  indexed by %lu", (unsigned long)k);
    size_t newSize = acl_arraysize + 1;
    Acl *acl2 = new Acl[newSize];
    size_t i;
    for (i = 0; i < k; i++)
        acl2[i] = this->acl[i];
    acl2[k] = acl;
...
void Acls::insertAcl(const Acl& acl)
{
    insertAcl(acl_arraysize, acl);
}

void Acls::eraseAcl(size_t k)
{
    if (k >= acl_arraysize) throw omnetpp::cRuntimeError("Array of size  indexed by %lu", (unsigned long)k);
    handleChange();
    size_t newSize = acl_arraysize - 1;
...
AclsDescriptor::AclsDescriptor() : omnetpp::cClassDescriptor(omnetpp::opp_typename(typeid(inet::Acls)), "inet::FieldsChunk")
{
    propertynames = nullptr;
}
...
unsigned int AclsDescriptor::getFieldTypeFlags(int field) const
{
    ...
    static unsigned int fieldTypeFlags[] = {
        FD_ISARRAY | FD_ISCOMPOUND,    // FIELD_acl
    };
...
int AclsDescriptor::findField(const char *fieldName) const
{
    omnetpp::cClassDescriptor *basedesc = getBaseClassDescriptor();
    int base = basedesc ? basedesc->getFieldCount() : 0;
    if (fieldName[0] == 'a' && strcmp(fieldName, "acl") == 0) return base+0;
    return basedesc ? basedesc->findField(fieldName) : -1;
}
...
int AclsDescriptor::getFieldArraySize(void *object, int field) const
{
    ...
    switch (field) {
        case FIELD_acl: return pp->getAclArraySize();
        default: return 0;
    }
}
...
std::string AclsDescriptor::getFieldValueAsString(void *object, int field, int i) const
{
    ...
    Acls *pp = (Acls *)object; (void)pp;
    switch (field) {
        case FIELD_acl: {std::stringstream out; out << pp->getAcl(i); return out.str();}
        default: return "";
    }
}
...
const char *AclsDescriptor::getFieldStructName(int field) const
{
    ...
    switch (field) {
        case FIELD_acl: return omnetpp::opp_typename(typeid(Acl));
        default: return nullptr;
    };
}
...
void *AclsDescriptor::getFieldStructValuePointer(void *object, int field, int i) const
{
    ...
    Acls *pp = (Acls *)object; (void)pp;
    switch (field) {
        case FIELD_acl: return toVoidPtr(&pp->getAcl(i)); break;
        default: return nullptr;
    }
}
