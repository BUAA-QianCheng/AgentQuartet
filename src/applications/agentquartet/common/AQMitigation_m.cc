...
EXECUTE_ON_STARTUP(
    ...
    e->insert(STANDING_BY, "STANDING_BY");
)
...
{
    delete [] this->targetPrefix;
    this->targetPrefix = (other.targetPrefix_arraysize==0) ? nullptr : new omnetpp::opp_string[other.targetPrefix_arraysize];
...
void MitigationTarget::parsimUnpack(omnetpp::cCommBuffer *b)
{
    ...
    if (targetPrefix_arraysize == 0) {
        ...
        this->targetPrefix = new omnetpp::opp_string[targetPrefix_arraysize];
...
const char * MitigationTarget::getTargetPrefix(size_t k) const
{
    ...
    return this->targetPrefix[k].c_str();
}

void MitigationTarget::setTargetPrefixArraySize(size_t newSize)
{
    handleChange();
    omnetpp::opp_string *targetPrefix2 = (newSize==0) ? nullptr : new omnetpp::opp_string[newSize];
...
void MitigationTarget::setTargetPrefix(size_t k, const char * targetPrefix)
{
...
void MitigationTarget::insertTargetPrefix(size_t k, const char * targetPrefix)
{
    handleChange();
    if (k > targetPrefix_arraysize) throw omnetpp::cRuntimeError("Array of size  indexed by %lu", (unsigned long)k);
    size_t newSize = targetPrefix_arraysize + 1;
    omnetpp::opp_string *targetPrefix2 = new omnetpp::opp_string[newSize];
    size_t i;
    for (i = 0; i < k; i++)
        targetPrefix2[i] = this->targetPrefix[i];
...
void MitigationTarget::insertTargetPrefix(const char * targetPrefix)
{
...
unsigned int MitigationTargetDescriptor::getFieldTypeFlags(int field) const
{
    ...
    static unsigned int fieldTypeFlags[] = {
        FD_ISARRAY | FD_ISEDITABLE,    // FIELD_targetPrefix
...
std::string MitigationTargetDescriptor::getFieldValueAsString(void *object, int field, int i) const
{
    ...
    MitigationTarget *pp = (MitigationTarget *)object; (void)pp;
    switch (field) {
        case FIELD_targetPrefix: return oppstring2string(pp->getTargetPrefix(i));
...
bool MitigationTargetDescriptor::setFieldValueAsString(void *object, int field, int i, const char *value) const
{
    ...
    MitigationTarget *pp = (MitigationTarget *)object; (void)pp;
    switch (field) {
        case FIELD_targetPrefix: pp->setTargetPrefix(i,(value)); return true;
